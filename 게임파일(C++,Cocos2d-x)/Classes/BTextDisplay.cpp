...
void BTextDisplay::Init(CCLayer* pThisScene,int nLayer,Point AnchorPoint, string stdName, float fX, float fY, int nLineWidth, int nLineHeight, int nFontSize, ccColor3B Color, string stdFont, bool Ortho2D)
{
	m_Color = Color;
	m_stdFont = stdFont;
...
void BTextDisplay::SetText(string String)
{
	if(IsViewing() == false)
	{
		...
		CCLabelTTF * m_pText = CCLabelTTF::create("", m_stdFont, m_nFontSize, Size::ZERO);//new CEMText(m_stdFont, "");
		m_pText->setAnchorPoint(m_AnchorPoint);
...
{ 
	...
	if(m_bSkip == false)
	{
		if(m_bFastView == true || m_fNowProgressTime >= m_fProgressTime)
		{
			...
			if(m_nNowChar > 0)
			{
				if(m_String[m_nNowChar] == '/')
				{
					m_nNowChar++;
					NextLine();
				}
				else if(m_nNowChar < m_nMaxChar)
				{
					if(m_String[m_nNowChar] & 0x80 ) //!< 현재 한글이면
					{
						for(int i=0; i<m_nCodeAndroid; i++)
						{
							m_NowString.push_back(m_String[m_nNowChar+i]);
						}
						m_nNowChar += m_nCodeAndroid;
	...
	m_nNowLineMaxChar++;
			...
			(*it)->setString(m_NowString);//(*it)->SetOutPutString(m_ViewString);
		}
...
void BTextDisplay::NextLine()
{
	...
	list<CCLabelTTF*>::iterator it;
	it = m_pTextList.begin();
	CCLabelTTF * m_pText = CCLabelTTF::create("",m_stdFont, m_nFontSize);//new CEMText(m_stdFont, " ");
	m_pText->setAnchorPoint(m_AnchorPoint);
	m_pText->setColor(ccColor3B(m_Color.r, m_Color.g, m_Color.b));
	m_pText->setPosition(m_stPos.x, m_stPos.y - (m_nNowLine * m_nLineHeight));
	//m_pText->setScale(m_stSize.x,m_stSize.y);
